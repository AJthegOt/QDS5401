...
table_one_analytic <- print(table_one_analytic_sample, 
      nonnormal = c("anxiety_score_pre", "anxiety_score_post"),
      cramVars = "grade_group")
```
...
barplot_input <- filtered_data_inclusion %>%
  group_by(race) %>%
  summarise(
    n = n(),
...
```{r}
anxiety_summary <- filtered_data_inclusion %>%
...
ggplot(filtered_data_inclusion, mapping = aes(x = race, y = anxiety_change, color = race)) +
  geom_boxplot() +
  labs(
    x = "Racial Group",
